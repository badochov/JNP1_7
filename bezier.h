...
namespace bezier {
    namespace types {
        ...
        struct List {
            ...
            static List<T> *get_empty() {
                return nullptr;
            }

            static bool is_empty(const List<T> *list) {
                return list == nullptr;
            }

            static bool contains(const List<T> *list, const T &el) {
                return !is_empty(list) && (list->el == el || contains(list->tail, el));
            }

            static T nth(const List<T> *list, size_t n) {
                return n == 0 ? list->el : nth(list, n - 1);
            }

            static List<T> *set_nth(const List<T> *list, size_t n, T &el) {
                return n == 0 ? new List<T>(el, list->tail) : set_nth(list, n - 1, el);
            }

            List(T _el, const List<T> *_tail) : el(std::move(_el)), tail(_tail) {
...
//            }
//
//            List2D(T _el, const List<T> *_tail) : el(std::move(_el)), tail(_tail) {
//
//            }
//        };
    ...
    inline types::point_function_t ConvexArc() {
        static const types::point_function_t convex_arc =
                detail::four_points_function(constants::zero_pos, types::point_2d(constants::ARC, 1),
    ...
    inline types::point_function_t ConcaveArc() {
        static const types::point_function_t concave_arc =
                detail::four_points_function(constants::zero_pos, types::point_2d(0, 1 - constants::ARC),
    ...
    inline types::point_function_t MovePoint(types::point_function_t f, types::node_index_t i,
    ...
    inline types::point_function_t Rotate(types::point_function_t f, types::real_t a) {
    ...
    inline types::point_function_t Scale(types::point_function_t f, types::real_t x, types::real_t y) {
    ...
    inline types::point_function_t Translate(types::point_function_t f, types::real_t x, types::real_t y) {
        return [f = std::move(f), x, y](types::node_index_t idx) {
            const types::point_2d v(x, y);
    ...
    inline types::point_function_t Concatenate(types::point_function_t f1, types::point_function_t f2) {
    ...
    class P3CurvePlotter {
        ...
        using should_be_printed_t = types::List<point>;
        should_be_printed_t *should_be_printed;
        size_t resolution;
        ...
        void print_column(size_t column, size_t row, std::ostream &os, char fg, char bg) const {
            os << (types::List<point>::contains(should_be_printed, {column, row}) ? fg : bg);
            if (column != resolution - 1) {
                print_column(column + 1, row, os, fg, bg);
            }
        }
        ...
        should_be_printed_t *add_segment_point(const std::function<types::point_2d(types::real_t)> &f, size_t i,
                                               size_t size, should_be_printed_t *should) {
            const types::real_t t = get_coord(i, size);
            const types::point_2d point = f(t);
            should_be_printed_t *print = in_area(point) ? new should_be_printed_t(create_point(point),
                                                                                  should) : should;
        ...
        should_be_printed_t *process_segment(const types::point_function_t &f, size_t segment, size_t segments,
                                             size_t size, should_be_printed_t *should) {
            const size_t segment_size = std::ceil((types::real_t) size / segments);

            should_be_printed_t *print = add_segment_point([&](types::real_t t) { return (*this)(f, t, segment); }, 0,
                                                           segment_size, should);

            return segments == 1 ? print : process_segment(f, segment + 1, segments - 1, size - segment_size, print);
    ...
    public:
        explicit P3CurvePlotter(const types::point_function_t &f, size_t segments = 1,
            ...
            const size_t size = resolution * resolution;
            should_be_printed = process_segment(f, 0, segments, size, types::List<point>::get_empty());
        }
